...
static LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
        ...
        case WM_SIZE:
            D3DManagerWithD2D::GetInstance().Resize(LOWORD(lParam), HIWORD(lParam));
            return 0;
        ...
        case WM_DESTROY:
			D3DManagerWithD2D::GetInstance().Destroy();
            PostQuitMessage(0);
            return 0;
    }
        ...
        )
{
    ...
    // create d3d
	if (!D3DManagerWithD2D::GetInstance().CreateDevice())return 2;
    ...
    while (true)
    {
        if (PeekMessage (&msg,NULL,0,0,PM_NOREMOVE))
        {
            ...
            // clear render target
            D3DManagerWithD2D::GetInstance().Render(hWnd);
        }
    }
